...
function Message({ message }: { message: MessageRecord }) {
  ...
  const contentWithSources = message.content.replace(
    /\[Source: (.*?)\]/g,
    (match, source) => {
      const url = `/${source.replace(/\\/g, '/').replace(/\.mdx?$/, '')}`;
      return `<a href="${url}" target="_self">${source}</a>`;
    },
  );
  ...
  return (
    <div>
      ...
      <div className="prose text-sm">
        <div dangerouslySetInnerHTML={{ __html: contentWithSources.replace(/\n/g, '<br />') }} />
      </div>
      {references.length > 0 ? (
...
export default function AISearch(props: DialogProps) {
  const [type, setType] = useState<EngineType>('ai-sdk');
          ...
          >
            ...
            <div className="flex flex-row gap-2 items-center bg-fd-muted text-fd-muted-foreground px-3 py-1.5 rounded-b-xl border-b border-x shadow-lg justify-between flex-wrap">
              <div className="flex flex-row items-center">
                <button
                  className={cn(
                    typeButtonVariants({ active: mode === 'search' }),
                  )}
                  onClick={() => {
                    setMode('search');
                  }}
                ...
                <button
                  className={cn(
                    typeButtonVariants({ active: mode === 'agent' }),
                  )}
                  onClick={() => {
                    setMode('agent');
                  }}
