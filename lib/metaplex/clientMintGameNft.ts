...
async function loadMetadataSerializers() {
  ...
  const modME = await import(
    "@metaplex-foundation/mpl-token-metadata/dist/src/generated/instructions/createMasterEditionV3.js"
  );
  const types = await import(
    "@metaplex-foundation/mpl-token-metadata/dist/src/generated/types/index.js"
  );
...
export async function clientMintGameNft(params: {
  connection: Connection;
  walletPublicKey: string;
  sendAndConfirm: (tx: Transaction) => Promise<string>;
  gameId: string;
}): Promise<{ mintAddress: string; signature: string; metadataOk: boolean }> {
  ...
  try {
    ...
    const uri =
      "https://copper-gigantic-kite-657.mypinata.cloud/ipfs/bafkreicmeavqcbhkusk5viagwdjiiuyruthemxk2f5o3uohevf4oco26pa";
    ...
    const meData = meSerializer.serialize({
      maxSupply: { __option: "Some", value: BigInt(0) },
    });
    ...
    console.warn(
      "[clientMintGameNft] metadata build failed – proceeding without metadata",
      e
    );
  }
