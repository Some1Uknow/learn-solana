...
<InfoCallout title="Why The Compiler Pushes Back">
...
<ConceptCards concepts={[
  {
    term: "Move",
    definition: "An ownership transfer from one binding or function call to another. After a move, the old binding is no longer valid."
  },
  {
    term: "Borrow",
    definition: "A temporary reference to data without taking ownership. Borrowing is how Rust lets you use data while keeping ownership clear."
  },
  {
    term: "Mutable Borrow",
    definition: "A temporary exclusive reference that allows mutation. Rust only allows it when nothing else is reading or mutating the same value."
  },
  {
    term: "Slice",
    definition: "A borrowed view into part of a collection or string, such as `&str`. Slices let you point at existing data without allocating a new owned value."
  }
]} />
...
flowchart LR
  A[Value is created]
  B[Owned by one binding]
  C[Moved to new owner]
  D[Borrowed immutably]
  E[Borrowed mutably]
  F[Old binding invalid after move]
...
You can compress most of this lesson into three rules.
...
<WarningCallout title="Using a value after it moved">
...
<WarningCallout title="Mixing immutable and mutable borrows carelessly">
...
<WarningCallout title="Cloning when borrowing would be enough">
...
<QuickCheck
  id="ownership-borrowing-move-vs-borrow"
  question="When should a function take `&String` or `&str` instead of `String`?"
  options={[
    "When the function only needs to read the text and should not become the owner",
    "When Rust requires every string parameter to be borrowed",
    "When you want the function to drop the string automatically",
    "When the function needs to mutate the string and return it"
  ]}
  correctIndex={0}
  explanation="If the function only needs read-only access, borrowing is usually the right model. That keeps the original owner valid and avoids unnecessary moves or clones."
/>

<QuickCheck
  id="ownership-borrowing-exclusive-access"
  question="Why does Rust reject a mutable borrow while immutable borrows are still active?"
  options={[
    "Because mutable borrows are slower than immutable ones",
    "Because Rust requires mutation to happen only when access is exclusive",
    "Because immutable borrows always turn into copies",
    "Because `&mut` is only allowed inside `impl` blocks"
  ]}
  correctIndex={1}
  explanation="Rust uses exclusive mutable access to prevent inconsistent reads and data races. If something is being mutated, nothing else should be reading or mutating that value at the same time."
/>
...
That is the capability you should leave this lesson with.
...
<UpNextCard
  nextTitle="Ownership and Borrowing Practice"
  nextDescription="Work through focused exercises that force you to distinguish copying, moving, immutable borrowing, mutable borrowing, and unnecessary cloning."
  nextHref="/learn/rust-foundations/ownership-and-borrowing-practice"
  nextReadingTime={10}
  prevTitle="Rust Syntax Practice"
  prevHref="/learn/rust-foundations/rust-syntax-practice"
