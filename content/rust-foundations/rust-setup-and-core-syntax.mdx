...
<ConceptCards concepts={[
  {
    term: "rustup",
    definition: "The tool that installs and manages Rust toolchains on your machine."
  },
  {
    term: "Cargo",
    definition: "Rust's project manager and build tool. It creates projects, compiles code, runs binaries, and later manages dependencies."
  },
  {
    term: "mut",
    definition: "The keyword that makes reassignment explicit. In Rust, variables stay immutable unless you opt into mutation."
  },
  {
    term: "Expression",
    definition: "A piece of code that produces a value. Rust functions and `if` blocks rely on expressions more heavily than many beginners expect."
  }
]} />
...
flowchart LR
  A[Install rustup]
  B[Verify rustc and cargo]
  C[cargo new]
  D[Edit src/main.rs]
  E[cargo run]
  F[Read compiler output]
  G[Adjust code]
...
<WarningCallout title="Forgetting `mut`">
...
<WarningCallout title="Accidentally returning `()` instead of a value">
This fails:
...
<WarningCallout title="Mixing incompatible numeric types">
...
<QuickCheck
  id="rust-setup-cargo-role"
  question="What is the most important reason to start with `cargo new` instead of a random `.rs` file?"
  options={[
    "Cargo gives you the right project structure and the normal Rust build workflow from the start",
    "Rust code only compiles if Cargo creates the file",
    "Cargo automatically fixes ownership bugs",
    "Rust projects cannot contain more than one source file unless Cargo creates them"
  ]}
  correctIndex={0}
  explanation="The lesson is teaching workflow, not just syntax. `cargo new` gives you the normal project structure and makes `cargo run` part of your habit immediately."
/>

<QuickCheck
  id="rust-setup-expression-return"
  question="Why does adding a semicolon to the last line of `calculate_remaining_balance` change the result?"
  options={[
    "Because the semicolon makes Rust evaluate the line twice",
    "Because the semicolon turns the final expression into a statement, so the function stops returning that value",
    "Because semicolons are only allowed in macros",
    "Because return types only work on immutable variables"
  ]}
  correctIndex={1}
  explanation="Rust functions can return the final expression implicitly. Add a semicolon, and you no longer return that value. That is one of the first expression-vs-statement rules worth making automatic."
/>
...
<UpNextCard
  nextTitle="Rust Syntax Practice"
  nextDescription="Use variables, functions, control flow, loops, and input handling in small exercises before the ownership lesson changes the mental model."
  nextHref="/learn/rust-foundations/rust-syntax-practice"
  nextReadingTime={10}
  prevTitle="Rust for Solana"
  prevHref="/learn/rust-foundations"
