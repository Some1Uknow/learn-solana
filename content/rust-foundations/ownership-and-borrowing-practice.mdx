...
<QuickCheck
  id="ownership-practice-clone-check"
  question="When is reaching for `.clone()` the right first move in these exercises?"
  options={[
    "When the compiler complains about ownership and you want the error gone quickly",
    "Only after you can explain why borrowing is not enough for the behavior you want",
    "Whenever a function takes a `String`",
    "Whenever you need to print a value twice"
  ]}
  correctIndex={1}
  explanation="The point of the page is to force the ownership decision into the open. Clone can be correct, but only after you know why a borrow or move does not match the job."
/>
...
<UpNextCard
  nextTitle="Structs and Methods in Rust"
  nextDescription="Move from standalone ownership decisions into named data types with behavior, where `&self`, `&mut self`, and `self` start shaping your API design."
  nextHref="/learn/rust-foundations/structs-and-methods"
  nextReadingTime={12}
  prevTitle="Understanding Rust Ownership and Borrowing"
  prevHref="/learn/rust-foundations/ownership-and-borrowing"
