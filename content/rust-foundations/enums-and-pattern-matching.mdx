...
<InfoCallout title="The Mental Model">
...
<ConceptCards concepts={[
  {
    term: "Enum",
    definition: "A type that can be exactly one of several valid variants at a time, with each variant carrying its own shape."
  },
  {
    term: "Variant",
    definition: "One possible state inside an enum, such as `Pending` or `Failed { error: String }`."
  },
  {
    term: "Pattern Match",
    definition: "A control-flow structure that inspects an enum variant and unpacks any data inside it."
  },
  {
    term: "Exhaustiveness",
    definition: "Rust's guarantee that a `match` must handle every variant, so control flow stays aligned with the data model."
  }
]} />
...
flowchart LR
  A[Enum value]
  B[Pending]
  C[Confirmed with data]
  D[Failed with data]
  E[match forces explicit handling]
...
<QuickCheck
  id="enums-theory-why-enums"
  question="Why is an enum often stronger than a struct full of flags and `Option` fields?"
  options={[
    "Because an enum prevents invalid state combinations by allowing exactly one variant at a time",
    "Because enums are always smaller in memory than structs",
    "Because Rust does not allow methods on structs with optional fields",
    "Because enums automatically implement error handling"
  ]}
  correctIndex={0}
  explanation="The big win is not style. It is correctness. Enums encode mutually exclusive states directly, so the type system blocks combinations that should never exist together."
/>

<QuickCheck
  id="enums-theory-match-vs-if-let"
  question="When is `match` a better fit than `if let`?"
  options={[
    "When every variant matters and you want the compiler to enforce complete handling",
    "Only when the enum contains named fields",
    "Only when the enum is generic",
    "Whenever you need to print a value"
  ]}
  correctIndex={0}
  explanation="Use `if let` when you only care about one case. Use `match` when the full set of states matters to correctness and the compiler should keep you honest."
/>
...
<UpNextCard
  nextTitle="Enums and Pattern Matching Practice"
  nextDescription="Build a small wallet manager that uses enums for account state, transaction outcomes, and explicit failure reasons instead of vague flags."
  nextHref="/learn/rust-foundations/enums-and-pattern-matching-practice"
  nextReadingTime={10}
  prevTitle="Wallet Ledger Build"
  prevHref="/learn/rust-foundations/wallet-ledger-build"
