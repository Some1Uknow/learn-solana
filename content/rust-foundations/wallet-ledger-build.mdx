...
<ConceptCards concepts={[
  {
    term: "LedgerEntry",
    definition: "One recorded wallet action, such as a deposit or withdrawal. It turns state changes into visible history."
  },
  {
    term: "Summary Method",
    definition: "A read-only method that turns the wallet state into a human-readable string without mutating anything."
  },
  {
    term: "Result",
    definition: "Rust's explicit success-or-failure type. `withdraw` should use it because insufficient balance and frozen state are expected failure paths."
  },
  {
    term: "CLI Loop",
    definition: "The repeated input-processing cycle that keeps the wallet program interactive without introducing UI complexity."
  }
]} />
...
<WarningCallout title="Putting too much logic in `main`">
...
<WarningCallout title="Using field writes everywhere instead of methods">
...
<WarningCallout title="Reaching for extra abstractions too early">
...
<QuickCheck
  id="wallet-ledger-withdraw-result"
  question="Why should `withdraw` return `Result<(), String>` instead of just changing the balance directly?"
  options={[
    "Because any method that mutates state must return `Result` in Rust",
    "Because withdrawal can fail for expected reasons like insufficient balance or a frozen wallet",
    "Because methods are not allowed to print errors directly",
    "Because `Result` is required whenever a struct owns a `Vec`"
  ]}
  correctIndex={1}
  explanation="Withdrawal is not guaranteed to succeed. Returning `Result` makes the failure path explicit and keeps the business rules in the wallet API instead of hiding them in `main`."
/>

<QuickCheck
  id="wallet-ledger-main-vs-impl"
  question="What is the main architectural lesson of this build?"
  options={[
    "Interactive programs should put all logic inside the input loop",
    "The data type should own the core state transitions, while `main` coordinates input and output",
    "All wallet state should stay public so the CLI can edit it freely",
    "CLI projects only work well if everything lives in one giant function"
  ]}
  correctIndex={1}
  explanation="The build is teaching separation of responsibilities at a beginner-friendly scale. `main` handles the menu flow. `Wallet` owns the business rules and state changes."
/>
...
<UpNextCard
  nextTitle="Enums and Pattern Matching in Rust"
  nextDescription="Move from single-struct modeling into state variants, explicit branching, and the enum patterns that show up constantly in real Rust and Solana code."
  nextHref="/learn/rust-foundations/enums-and-pattern-matching"
  nextReadingTime={12}
  prevTitle="Struct and Method Practice"
  prevHref="/learn/rust-foundations/structs-and-methods-practice"
