...
<WarningCallout title="Keep The Scope Tight">
...
<QuickCheck
  id="enums-practice-wallet-error"
  question="Why is `WalletError` worth modeling as an enum instead of returning random strings from every branch?"
  options={[
    "Because enums make failure reasons explicit and keep the set of expected errors coherent",
    "Because Rust cannot print string errors from functions",
    "Because `Result` only works if the error type is an enum",
    "Because enums automatically log failures to the console"
  ]}
  correctIndex={0}
  explanation="The point is not ceremony. A dedicated error enum names the valid failure reasons and keeps the operation contract honest instead of scattering unrelated strings through the code."
/>

<UpNextCard
  nextTitle="Collections and String Handling in Rust"
  nextDescription="Move from single values and explicit state models into vectors, hash maps, owned strings, borrowed string slices, and basic text parsing."
  nextHref="/learn/rust-foundations/collections-and-string-handling"
  nextReadingTime={12}
  prevTitle="Enums and Pattern Matching in Rust"
  prevHref="/learn/rust-foundations/enums-and-pattern-matching"
