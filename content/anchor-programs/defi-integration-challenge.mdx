...
```mermaid
flowchart LR
    A[Payer funds vault] --> B[Settlement PDA becomes Funded]
    B --> C[Program computes fee and net amount]
    C --> D[Vault authority PDA signs token CPIs]
    D --> E[Treasury receives fee]
    D --> F[Recipient receives proceeds]
    E --> G[Settlement marked Settled]
    F --> G
```
...
<WarningCallout title="Do Not Hand-Wave Vault Authority">
...
<QuickCheck
  id="anchor-capstone-fee-rule"
  question="Why should the fee calculation and treasury route live inside the program instead of only in the client?"
  options={[
    "Because fee logic is part of the trust boundary, and the program must enforce who gets paid and how much",
    "Because clients are unable to read token mint decimals",
    "Because treasury token accounts cannot be passed into a transaction",
    "Because PDA signer seeds only work when fee math is off-chain first",
  ]}
  correctIndex={0}
  explanation="If the protocol takes a fee, the program must compute and route it itself. Otherwise the most important settlement rule is not really enforced."
/>

<QuickCheck
  id="anchor-capstone-replay-protection"
  question="What actually prevents the settlement from executing twice in the capstone design?"
  options={[
    "The on-chain settlement status changes after success, so later calls fail the state check",
    "The frontend hides the settle button after the first click",
    "The vault token account deletes itself automatically",
    "The CPI helper remembers whether it already ran",
  ]}
  correctIndex={0}
  explanation="Replay protection is a state-machine rule. The program must update status and reject later attempts."
/>

<UpNextCard
  nextTitle="Anchor Programs"
  nextDescription="Return to the module overview and use it as the launch point for another pass, a project build, or a full section review with the new mental models in place."
  nextHref="/learn/anchor-programs"
  nextReadingTime={4}
  prevTitle="Anchor Development Ergonomics"
  prevHref="/learn/anchor-programs/anchor-ergonomics"
