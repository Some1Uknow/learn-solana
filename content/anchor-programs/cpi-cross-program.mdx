  ...
  concepts={[
    {
      term: "CPI",
      definition:
        "A cross-program invocation where your program calls an instruction on another Solana program.",
    },
    {
      term: "CpiContext",
      definition:
        "The Anchor wrapper that packages the target program and the account set for a CPI.",
    },
    {
      term: "CPI Account Struct",
      definition:
        "The account layout expected by the callee, such as `Transfer` or `TransferChecked`.",
    },
    {
      term: "with_signer",
      definition:
        "The mechanism that lets the runtime verify PDA signer seeds and treat the PDA as a signer for the CPI.",
    },
  ]}
/>
...
```mermaid
flowchart LR
    A[Your instruction handler] --> B[Build CPI account struct]
    B --> C[Create CpiContext]
    C --> D[Optional with_signer seeds]
    D --> E[Call callee program]
    E --> F[State changes in callee]
```
...
<InfoCallout title="PDA Signing Is Still Verified">
...
<QuickCheck
  id="anchor-cpi-three-parts"
  question="What are the three core ingredients you should identify in any CPI?"
  options={[
    "The callee program, the accounts it expects, and the instruction data or arguments",
    "The client wallet adapter, the React hook, and the RPC endpoint",
    "The rent exemption, the blockhash, and the transaction fee",
    "The seed namespace, the event type, and the IDL version",
  ]}
  correctIndex={0}
  explanation="If you can name the program, the expected accounts, and the instruction data, the CPI structure stops feeling magical."
/>

<QuickCheck
  id="anchor-cpi-signer-rule"
  question="Why must the signer seeds passed into `with_signer(...)` match the PDA rule in the account context?"
  options={[
    "Because the runtime only lets the PDA sign if those seeds derive the same validated PDA",
    "Because Anchor rewrites the account context from the signer seeds at runtime",
    "Because `with_signer(...)` is only a logging helper for the IDL",
    "Because signer seeds are only checked in tests",
  ]}
  correctIndex={0}
  explanation="PDA signing is still verified. If the signer seeds do not match the validated derivation rule, the PDA cannot authorize the CPI."
/>

<UpNextCard
  nextTitle="Token Minting and Supply Control"
  nextDescription="Use PDAs, token CPIs, and on-chain business rules together in one guided project where the program controls mint authority and supply."
  nextHref="/learn/anchor-programs/token-minting-project"
  nextReadingTime={15}
  prevTitle="SPL Token Integration with Anchor"
  prevHref="/learn/anchor-programs/spl-token-integration"
